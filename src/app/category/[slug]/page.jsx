...
import { Star, Heart, ShoppingCart, Eye, Filter, SortAsc, SortDesc, Zap, Crown, Gift, TrendingUp, Sparkles, Car } from 'lucide-react';
...
export default function CategoryPage({ params }) {
  ...
  // Map alternative slugs to canonical ones
  const canonicalSlug = slug === 'sports' ? 'sports-fitness' 
    : slug === 'home' ? 'home-garden' 
    : slug;

  // Validate slug and format category name
  const categoryName = canonicalSlug
    ? canonicalSlug.charAt(0).toUpperCase() + canonicalSlug.replace('-', ' & ').slice(1)
    : 'Unknown Category';
  ...
  // Hardcoded products including all categories
  const allProducts = [
  ...
  const filteredProducts = allProducts
    .filter((product) => product.category.toLowerCase() === (canonicalSlug || ''))
  ...
  const CategoryIcon = canonicalSlug === 'electronics' ? Zap 
    : canonicalSlug === 'fashion' ? Crown 
    : canonicalSlug === 'home-garden' ? Gift 
    : canonicalSlug === 'sports-fitness' ? TrendingUp 
    : canonicalSlug === 'beauty' ? Sparkles 
    : canonicalSlug === 'automotive' ? Car 
    : Zap;
      ...
      <section className="relative py-20">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center space-x-3 mb-4">
            <CategoryIcon className="w-8 h-8 text-yellow-400 animate-pulse" />
            <h1 className="text-5xl font-black text-white">
              {categoryName} <span className="bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent">Collection</span>
            </h1>
            <CategoryIcon className="w-8 h-8 text-yellow-400 animate-pulse" />
          </div>
          <p className="text-xl text-white/90 max-w-2xl mx-auto">
            Discover premium {categoryName.toLowerCase()} products in our curated collection
          </p>
        </div>
      </section>
      ...
      <section className="py-20 bg-gray-900 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filteredProducts.length === 0 ? (
            <p className="text-center text-white/90 text-xl">No products found matching your filters.</p>
          ) : (
